...
def get_prime_form(confounds, n_buckets, sorted_confounds=None):
    """Uses prime numbers to speed up datapoint matching.

    Each bucket gets a prime number, and each datapoint is assigned a product
    of these primes. These are then matched with one another.
    """
    ...
    for i in range(confounds.shape[0]):
        if isinstance(confounds[i, 0], str):
            ...
            for kk in range(
                0,
                sorted_confounds.shape[1],
                int(np.ceil(sorted_confounds.shape[1] / float(n_buckets[i]))),
            ):
            ...
            buckets_v = (
                np.array(range(n_buckets[i] + 1)) / float(n_buckets[i])
...
def get_none_array(classes, confounds):
    """
    Returns a boolean array that is true if either classes or confounds has a
    None or NaN value anywhere at the given index.
    """
    has_none = np.zeros(classes.shape, dtype=bool)
    for i in range(confounds.shape[1]):
        #             print(classes[i])
        if classes[i] is None or classes[i] == np.nan:
            has_none[i] = True
        for j in range(confounds.shape[0]):
            if confounds[j, i] is None or confounds[j, i] == np.nan:
                has_none[i] = True
            try:
...
def class_balance(
    ...
    and confounds (as floats and strings, or just objects, in a 2D numpy
    array). plim is the max p-value, in a nonparametric statistical test, at
    which discretization stops and enough buckets have been reached. If
    recurse is set to True, this method calls itself recursively on excluded
    data, though this doesn't guarantee that the final p values for
    continuous covariates will be up to snuff.
    ...
    #     gg = str(
    #       classes[i]) + " " + \
    #       str(confounds[1,i]) #+ " " + str(confounds[1,i]
    #     )
    #     if gg not in ff:
    #         ff[gg] = gg
    #         print(gg)
    # for i in range(confounds.shape[0]):
    #     print(
    #         str(confounds[i,0]) + " is str: " + \
    #         str(isinstance(confounds[i,0],str))
    #     )
    ...
    for i in range(confounds.shape[0]):
        if (
            len(confounds.shape) > 1
            and confounds.shape[1] > 0
            and isinstance(confounds[i, 0], str)
        ):
    ...
    while min(p_vals) < plim and np.sum(selection) > 0:
        ...
        for i in rr:
            ...
            if not isinstance(confounds[i, 0], str):
                ...
                ts = [
                    confounds[i, np.logical_and(selection, classes == j)]
                    for j in range(len(unique_classes))
                ]
                # print(ts)
                # Makes sure there are at least five instances of each
                # class remaining
                if np.any(
                    list(map(lambda k: len(k) < 5, ts))
                ):  # or len(ts) <= 1:
                    selection = np.zeros(classes.shape, dtype=bool)
                    break
                # print("HERE")
    ...
    if np.sum(selection) > 40 and confounds[:, ~selection].shape[1] > 0:
        recurse_selection = integrate_arrs(
            selection,
            class_balance(
                classes[~selection],
                confounds[:, ~selection],
...
def separate_set(selections, set_divisions=[0.5, 0.5], IDs=None):
    ...
    # IDs = get_prime_form(
    #     IDs,
    #     [len(np.unique(IDs[x,:])) for x in range(IDs.shape[0])],
    #     np.sort(IDs,axis=1)
    # )
    ...
    for i in rr:
        if not selections[i]:
            continue
        is_none = IDs[i] is None or IDs[i] == "NULL"
        ...
        for j in range(len(set_divisions)):
            if (
                np.sum(totals) == 0
                or totals[j] / np.sum(totals) < set_divisions[j]
            ):
                break
