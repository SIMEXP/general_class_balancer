...
c1 = np.array(
    [norm(1, 2, int(N / 3)), norm(0.5, 2, int(N / 3)), norm(2, 1, int(N / 3))],
    dtype=object,
)
c2 = np.array(
    [
...
c4 = np.array(
    [[np.random.choice(["a", "b", "c"]) for x in range(N)]], dtype=object
)
confounds = np.concatenate((confounds, c4), axis=0)

classes = np.array(
    [0 for x in range(int(N / 2))] + [1 for x in range(int(N / 2))]
)
classes = np.array(
    [0 for x in range(int(N / 3))]
...
if show_balanced:
    plt.hist(
        confounds[cnum, np.logical_and(selection, classes == 0)],
        bins=bins,
        fc=(0, 0, 1, 0.5),
    )
    plt.hist(
        confounds[cnum, np.logical_and(selection, classes == 1)],
        bins=bins,
        fc=(0, 1, 0, 0.5),
    )
    plt.hist(
        confounds[cnum, np.logical_and(selection, classes == 2)],
        bins=bins,
        fc=(1, 0, 0, 0.5),
    )
else:
